...
在編輯器展開「04 網站模型預覽設定」，可獨立設定攝影機水平角度、仰俯角、距離及播放動畫，並即時預覽。預設自動選擇 idle，沒有 idle 時靜止，也能指定任一動畫或不播放。按「儲存到作品集」保存設定；單純匯出 PNG 不會儲存預覽設定。這些設定不影響卡片構圖。訪客使用儲存的固定角度，不能旋轉、平移或縮放。關閉視窗會停止播放並移除預覽元件。舊卡片缺少設定時使用獨立預設角度（水平 25°、仰角 15°、距離 115%）與 idle。
